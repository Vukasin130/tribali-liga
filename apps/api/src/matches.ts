...
export async function listLiveMatches() {
  ...
  return mutateDatabase((db) => (db.matches as any[])
    .filter((match) => match.status === "live")
    .map((match) => enrichMatch(db, match)));
}

export async function getMatchDetail(matchId: string, user?: Actor | null) {
  if (hasDatabase()) return getMatchDetailDb(matchId, user);
  return mutateDatabase((db) => {
    const match = findById(db.matches as any[], matchId, "Utakmica nije pronadjena.");
    return {
      ...enrichMatch(db, match),
      lineups: (db.matchLineups as any[]).filter((item) => item.matchId === matchId),
      events: (db.matchEvents as any[])
        .filter((item) => item.matchId === matchId)
        .sort((a, b) => Number(a.minute || 0) - Number(b.minute || 0)),
      playerStats: (db.playerMatchStats as any[]).filter((item) => item.matchId === matchId),
      media: (db.mediaLinks as any[]).filter((item) => item.matchId === matchId),
      predictions: { home: 0, draw: 0, away: 0, total: 0, homePercent: 0, drawPercent: 0, awayPercent: 0, userPick: "" }
    };
  });
}

export async function submitMatchPrediction(matchId: string, payload: any, user: Actor | null) {
  if (hasDatabase()) return submitMatchPredictionDb(matchId, payload, user);
  throw httpError(501, "Predikcije zahtevaju bazu podataka.");
}

export async function setMatchStatus(matchId: string, payload: any, actor: Actor) {
...
export async function setMatchPeriod(matchId: string, payload: any, actor: Actor) {
  if (hasDatabase()) return setMatchPeriodDb(matchId, payload, actor);
  throw httpError(501, "Live sat zahteva bazu podataka.");
}
...
export async function upsertLineup(matchId: string, payload: any, actor: Actor) {
  if (hasDatabase()) return upsertLineupDb(matchId, payload, actor);
  return mutateDatabase((db) => {
    findById(db.matches as any[], matchId, "Utakmica nije pronadjena.");
    const players = Array.isArray(payload.players) ? payload.players : [];
    if (!players.length) throw httpError(400, "Sastav mora imati igrace.");
    db.matchLineups = (db.matchLineups as any[]).filter((item) => item.matchId !== matchId);
    ...
    (db.matchLineups as any[]).push(...rows);
    audit(db, actor, "match.lineup.upsert", "match", matchId, { count: rows.length });
    return rows;
  });
}

export async function addMatchEvent(matchId: string, payload: any, actor: Actor) {
  ...
  return mutateDatabase((db) => {
    ...
    (db.matchEvents as any[]).push(event);
    match.updatedAt = timestamp();
...
export async function setMatchMedia(matchId: string, payload: any, actor: Actor) {
  if (hasDatabase()) return setMatchMediaDb(matchId, payload, actor);
  return mutateDatabase((db) => {
    findById(db.matches as any[], matchId, "Utakmica nije pronadjena.");
    const kind = String(payload.kind || "youtube");
    const mediaLinks = db.mediaLinks as any[];
    const existing = mediaLinks.find((item) => item.matchId === matchId && item.kind === kind);
    const item = existing || {
      id: randomUUID(),
      createdAt: timestamp(),
      matchId,
    ...
    if (!existing) mediaLinks.push(item);
    audit(db, actor, "match.media.upsert", "mediaLink", item.id, { matchId, kind });
    return item;
  });
}

export async function createMatch(payload: any, actor: Actor) {
...
function applyEventToMatch(db: any, match: any, event: any) {
  if (event.type === "goal") {
...
function incrementStat(db: any, event: any, field: string, amount: number, fantasyDelta: number) {
  if (!event.playerId || !event.teamId) return;
  const playerMatchStats = db.playerMatchStats as any[];
  let stat = playerMatchStats.find((item) => item.matchId === event.matchId && item.playerId === event.playerId);
  if (!stat) {
    stat = {
      id: randomUUID(),
      createdAt: timestamp(),
...
function enrichMatch(db: any, match: any) {
  const homeTeam = (db.teams as any[]).find((team) => team.id === match.homeTeamId);
  const awayTeam = (db.teams as any[]).find((team) => team.id === match.awayTeamId);
  const competition = (db.competitions as any[]).find((item) => item.id === match.competitionId);
  return {
    ...match,
...
function validateEvent(event: { type: string; minute: number }) {
...
function audit(db: any, actor: Actor, action: string, entityType: string, entityId: string, metadata: unknown) {
  db.auditLogs.push({
    id: randomUUID(),
    createdAt: timestamp(),
    updatedAt: timestamp(),
