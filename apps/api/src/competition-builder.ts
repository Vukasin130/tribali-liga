...
const DEFAULT_PHASES: Record<string, PhaseDefaults[]> = {
  league: [
...
export async function getCompetitionSetup(competitionId: string) {
...
export async function configureCompetition(competitionId: string, payload: ConfigurePayload, actor: Actor) {
...
export async function addTeamsToCompetition(competitionId: string, payload: { teams?: any[]; syncFantasy?: boolean }, actor: Actor) {
...
export async function cloneCompetitionTeams(competitionId: string, payload: CloneTeamsPayload, actor: Actor) {
  ...
  const result = await transaction(async (client) => {
    ...
    const params: unknown[] = [sourceCompetitionId];
    let teamFilter = "";
    ...
    for (const team of sourceTeams.rows) {
      const saved = await client.query(
        `insert into public.teams
           (competition_id, club_id, name, short_name, logo_url, group_name, placement, is_active, source_team_id)
         values ($1, $2, $3, $4, $5, $6, '', true, $7)
         returning *`,
        [competitionId, team.club_id || null, team.name, team.short_name, team.logo_url, payload.keepGroups ? team.group_name : "", team.id]
      );
    ...
    if (includePlayers) {
      ...
      for (const player of sourcePlayers.rows) {
        ...
        // Same person, new roster row - never a new players identity (that was
        // the bug: a real person got a brand-new players row per competition).
        await client.query(
...
export async function addClubToCompetition(competitionId: string, payload: { clubId?: string; includePlayers?: boolean; groupName?: string; syncFantasy?: boolean }, actor: Actor) {
  const clubId = requiredText(payload.clubId, "Klub je obavezan.");
  const includePlayers = payload.includePlayers !== false;

  const result = await transaction(async (client) => {
    await ensureCompetition(client, competitionId);
    const club = await client.query("select * from public.clubs where id = $1 and is_active = true", [clubId]);
    if (!club.rows[0]) throw httpError(404, "Klub nije pronadjen.");

    const existing = await client.query(
      "select id from public.teams where club_id = $1 and competition_id = $2 and is_active = true limit 1",
      [clubId, competitionId]
    );
    if (existing.rows[0]) throw httpError(409, "Ovaj klub je vec dodat u ovo takmicenje.");

    // Most recent OTHER team-row for this club - seeds short_name/logo and the
    // player roster to copy in. "Most recent" = latest competition start date,
    // falling back to the team row's own update time.
    const sourceTeam = await client.query(
      `select t.*
       from public.teams t
       left join public.competitions c on c.id = t.competition_id
       where t.club_id = $1 and t.competition_id is distinct from $2 and t.is_active = true
       order by c.starts_at desc nulls last, t.updated_at desc
       limit 1`,
      [clubId, competitionId]
    );
    const source = sourceTeam.rows[0] || null;
    ...
    const saved = await client.query(
      ...
      [
        ...
        clubId,
        club.rows[0].name,
        source?.short_name || club.rows[0].short_name || "",
        source?.logo_url || club.rows[0].logo_url || "",
        String(payload.groupName || "").trim(),
        source?.id || null
      ]
    );
    const newTeam = saved.rows[0];

    let playersCount = 0;
    if (includePlayers && source) {
      const sourcePlayers = await client.query(
        `select p.id
         from public.players p
         join public.team_rosters tr on tr.player_id = p.id and tr.is_active = true
      ...
      for (const player of sourcePlayers.rows) {
        // Same person, new roster row - never a new players identity.
        await client.query(
          "insert into public.team_rosters (player_id, team_id) values ($1, $2) on conflict do nothing",
          [player.id, newTeam.id]
        );
        playersCount += 1;
      }
    }
    ...
    return { team: newTeam, playersCount };
  });

  const fantasyPool = result.playersCount > 0 && payload.syncFantasy !== false
    ? await syncFantasyPlayerPool(competitionId, actor)
    : null;
...
export async function assignCompetitionGroups(competitionId: string, payload: { groupNames?: string[]; groupCount?: number; seeds?: string[] }, actor: Actor) {
...
export async function upsertScheduleSlots(competitionId: string, payload: { slots?: any[]; replace?: boolean }, actor: Actor) {
...
export async function generateCompetitionSchedule(competitionId: string, payload: GenerateSchedulePayload, actor: Actor) {
...
export async function advanceKnockoutPhase(competitionId: string, payload: any, actor: Actor) {
  const phaseCode = String(payload.phaseCode || "knockout").trim();
  ...
  const result = await transaction(async (client) => {
    ...
    return { champion: null as { teamId: string; teamName: string } | null, matches };
  });
...
async function upsertPhaseRules(client: PoolClient, phaseId: string, rules: any[]) {
...
async function getPhaseByCode(client: PoolClient, competitionId: string, code: string) {
...
async function loadTeamsForPhase(client: PoolClient, competitionId: string, phase: any) {
...
async function loadUsableSlots(client: PoolClient, competitionId: string, count: number, startAt: string, intervalMinutes: number, venue: string, pairings: any[] = []) {
  const existing = await client.query(
    `select *
     from public.competition_schedule_slots
  ...
  const nextTime = new Date(startAt).getTime();
...
function buildRoundRobin(teams: any[], legs: number) {
...
function groupTeamsForSchedule(teams: any[]) {
...
function roundRobinGroup(groupTeams: any[]) {
  const teams: (any | null)[] = [...groupTeams];
...
async function selectQualifiers(client: PoolClient, competitionId: string, count: number) {
...
function pairSeeds(qualifiers: any[]) {
  const pairs = [];
  let left = 0;
...
function knockoutRoundNumber(count: number): number {
...
function buildFormatSummary(formatType: string, phases: any[]): string {
...
function normalizeFormat(row: any) {
  return {
    id: row.id,
...
function normalizePhase(row: any) {
  return {
    id: row.id,
    competitionId: row.competition_id,
    code: row.code,
...
function normalizeScheduleSlot(row: any) {
  return {
    id: row.id,
    competitionId: row.competition_id,
    startsAt: row.starts_at,
...
function normalizeQualifier(row: any) {
  return {
    teamId: row.team_id,
