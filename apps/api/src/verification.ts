...
export async function getOwnProfile(user: Actor | null) {
...
export async function updateOwnProfile(payload: UpdateProfilePayload, user: Actor | null) {
  if (!user) throw httpError(401, "Moras biti ulogovan.");
...
export async function listMyVerificationRequests(user: Actor | null) {
  if (!user) throw httpError(401, "Moras biti ulogovan.");
  const result = await query(
...
interface CreateVerificationRequestPayload {
  cityId?: string;
  teamId?: string;
  playerId?: string;
  playerName?: string;
...
export async function createVerificationRequest(payload: CreateVerificationRequestPayload, user: Actor | null) {
  if (!user) throw httpError(401, "Moras biti ulogovan.");
...
export async function listVerificationRequests(filters: { status?: string } = {}) {
...
interface ReviewVerificationRequestPayload {
  status?: string;
  playerId?: string;
  teamId?: string;
  adminNote?: string;
...
export async function reviewVerificationRequest(id: string, payload: ReviewVerificationRequestPayload, actor: Actor | null) {
...
export async function getProfileArchive(user: Actor | null) {
  if (!user) throw httpError(401, "Moras biti ulogovan.");
  ...
  return {
    expiredStories: stories.rows.map((row: any) => ({
      id: row.id,
...
function normalizeVerificationRequest(row: any) {
  return {
    id: row.id,
    createdAt: row.created_at,
    updatedAt: row.updated_at,
...
async function audit(actor: Actor | null, action: string, entityType: string, entityId: string | null, metadata: Record<string, unknown>) {
  await query(
    `insert into public.audit_logs (actor_user_id, action, entity_type, entity_id, metadata)
