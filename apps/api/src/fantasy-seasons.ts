...
export async function createFantasyGameweek(payload: GameweekPayload, actor: Actor) {
...
export async function updateFantasyGameweek(gameweekId: string, payload: GameweekPayload, actor: Actor) {
  const allowed: string[] = [];
  const params: unknown[] = [];
  const push = (sql: string, value: unknown) => {
    params.push(value);
  ...
  // A round opening is the one gameweek transition worth a push - it's the moment fans
  // actually have something to do (pick/change a team). Only fires on draft/locked -> open,
  // never re-fires if the round was already open (e.g. renaming it mid-round).
  const isOpeningNow = payload.status === "open";
  let wasAlreadyOpen = false;
  if (isOpeningNow) {
    const current = await query("select status from public.fantasy_gameweeks where id = $1", [gameweekId]);
    wasAlreadyOpen = current.rows[0]?.status === "open";
  }
  ...
  const updated = normalizeGameweek(result.rows[0]);
  if (isOpeningNow && !wasAlreadyOpen) {
    sendAutomaticNotification(
      "Novo fantazi kolo je otvoreno!",
      `${updated.name} je otvoreno - sastavi ili izmeni svoj tim na vreme.`,
      { kind: "gameweek_open", gameweekId: updated.id }
    );
  }
  return updated;
}

// Replaces blind hand-typed gameweek dates with windows derived from the
// linked leagues' own real fixtures - one fantasy gameweek per real round
// number, spanning from the round's first kickoff to a few hours after its
// last match. Safe to re-run: a round already "locked"/"scoring"/"finished"
// keeps its recorded dates untouched (only draft/open rounds get refreshed),
// and re-running never creates duplicates (upserts by round name).
export async function autoGenerateFantasyGameweeks(seasonId: string, actor: Actor) {
  await getSeasonRow(seasonId);
  ...
  const matches = await query(
    `select round, scheduled_at
     from public.matches
     where competition_id = any($1::uuid[]) and round is not null
     order by round, scheduled_at`,
    [competitionIds]
  );
  if (!matches.rows.length) {
    throw httpError(400, "Povezane lige jos nemaju zakazane termine sa kolima - prvo napravi raspored utakmica.");
  }

  const byRound = new Map<number, number[]>();
  for (const row of matches.rows) {
    if (!byRound.has(row.round)) byRound.set(row.round, []);
    byRound.get(row.round)!.push(new Date(row.scheduled_at).getTime());
  }

  let created = 0;
  let refreshed = 0;
  for (const [round, times] of [...byRound.entries()].sort((a, b) => a[0] - b[0])) {
    const startsAt = new Date(Math.min(...times)).toISOString();
    const endsAt = new Date(Math.max(...times) + 3 * 60 * 60 * 1000).toISOString();
    const name = `Kolo ${round}`;
    const result = await query(
      `insert into public.fantasy_gameweeks (fantasy_season_id, name, starts_at, locks_at, ends_at, status)
       ...
       on conflict (fantasy_season_id, name) do update set
         ...
         ends_at = excluded.ends_at,
         updated_at = now()
       where public.fantasy_gameweeks.status in ('draft', 'open')
       returning (xmax = 0) as inserted`,
      [seasonId, name, startsAt, endsAt]
    );
    if (result.rows[0]?.inserted) created += 1;
    else if (result.rows[0]) refreshed += 1;
  }

  await audit(actor, "fantasy.season.gameweeks.autogenerate", "fantasySeason", seasonId, { created, refreshed, rounds: byRound.size });
  return { created, refreshed, total: byRound.size };
}
...
export async function scoreFantasySeasonGameweek(fantasyGameweekId: string, actor: Actor) {
  const result = await transaction(async (client) => {
...
function validGameweekStatus(status: unknown): string {
  const normalized = String(status || "").trim();
