...
export async function createNewsPostDb(payload: NewsPayload, actor: Actor) {
  const result = await query(
...
export async function updateNewsPostDb(id: string, payload: NewsPayload, actor: Actor) {
...
export async function deleteNewsPostDb(id: string, actor: Actor) {
...
export async function createStoryFolderDb(payload: StoryFolderPayload, actor: Actor) {
  const result = await query(
...
export async function updateStoryFolderDb(id: string, payload: StoryFolderPayload, actor: Actor) {
...
export async function deleteStoryFolderDb(id: string, actor: Actor) {
  const result = await query(
...
export async function createStoryDb(payload: StoryPayload, actor: Actor) {
...
export async function deleteStoryDb(id: string, actor: Actor) {
...
export async function markStoryViewedDb(storyId: string, user: Actor | null) {
...
export async function toggleStoryLikeDb(storyId: string, user: Actor | null) {
...
export async function getStoryStatsDb(storyId: string) {
...
export async function updateSponsorDb(payload: SponsorPayload, actor: Actor) {
  let id = payload.id;
  if (!id) {
    const existing = await query("select id from public.sponsors where kind = 'weekly' order by updated_at desc limit 1");
...
// A third, independent pool of sponsors (kind='general') - shown together as a
// logo strip across multiple screens, unlike the single weekly banner or the
// per-match picker list above. Supports edit/deactivate since it's a standing
...
function normalizeNews(row: any) {
  return {
    id: row.id,
...
function normalizeStoryFolder(row: any) {
  return {
    id: row.id,
    createdAt: row.created_at,
    updatedAt: row.updated_at,
...
function normalizeStory(row: any) {
  return {
...
function normalizeStoryView(row: any) {
  return {
    id: row.id,
    createdAt: row.created_at,
...
function normalizeStoryLike(row: any) {
  return {
    id: row.id,
    createdAt: row.created_at,
    storyId: row.story_id,
...
function normalizeSponsor(row: any) {
  return {
    id: row.id,
    createdAt: row.created_at,
    updatedAt: row.updated_at,
...
function mediaTypeOrNull(value: unknown): string | null {
  return ["image", "video", "youtube", "link"].includes(value as string) ? (value as string) : null;
}
