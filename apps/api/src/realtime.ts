...
import { listActiveStories } from "./content.ts";
import { getCurrentGoalPoll } from "./goal-polls.ts";
import { listLiveMatches } from "./matches.ts";
...
export async function getLiveOverview(user: Actor | null) {
...
export function getRealtimeConfig(user: Actor | null, params: RealtimeParams = {}) {
  if (!config.supabase.url || !config.supabase.publishableKey) {
    throw httpError(500, "Supabase realtime konfiguracija nije podesena.");
  }

  const matchId = optionalUuidFilter(params.matchId);
  const gameweekId = optionalUuidFilter(params.gameweekId);
  const pollId = optionalUuidFilter(params.pollId);
  const storyId = optionalUuidFilter(params.storyId);
...
function postgresEvent(table: string, event: string, filter = "") {
  return {
...
function optionalUuidFilter(value: unknown): string {
